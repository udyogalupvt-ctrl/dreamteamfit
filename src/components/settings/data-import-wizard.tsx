...
const VIEW: Record<ImportType, "/clients" | "/packages" | "/expenses"> = {
  clients: "/clients",
  packages: "/packages",
  trainers: "/packages",
  memberships: "/clients",
...
export function DataImportWizard() {
  ...
  const expenses = useLive(finance ? subscribeExpenses : null, [] as Expense[], [finance]);
  ...
  const unknown = useMemo(
    () => (parsed ? unknownNames(type, parsed.rows, mapping, ctx) : { packages: [], trainers: [] }),
    [parsed, type, mapping, packages.data, trainers.data],
  );
  const validated = useMemo(
    () => (parsed ? validateRows(type, parsed.rows, mapping, ctx) : []),
    [
      parsed,
      type,
      mapping,
  ...
  const counts = {
    ...
    valid: validated.filter((r) => !r.errors.length).length,
    invalid: validated.filter((r) => r.errors.length).length,
    dup: validated.filter((r) => r.duplicate).length,
  };
  ...
  const read = async (f: File, sh?: string) => {
    ...
    try {
      ...
      setParsed(p);
      setSheet(sh ?? p.sheetNames[0] ?? "");
      setMapping(suggestMapping(type, p.headers));
      setDupActions({});
      setStep(2);
    } catch (e) {
  ...
  const failuresFor = (r: ImportResult | null) => {
    ...
    const items = (
      r?.failures ?? validated.flatMap((v) => v.errors.map((e) => ({ row: v.row, ...e })))
    ).map((f) => ({ ...f, raw: rows.get(f.row) ?? {} }));
  ...
  const doImport = async () => {
    ...
    try {
      ...
      if (await findCompletedBatch(fp))
        throw new Error("This exact file was already imported. Nothing was written again.");
  ...
  const shown = validated.filter(
    (r) => filter === "all" || (filter === "invalid" ? r.errors.length : r.duplicate),
  );
    ...
    <section className="surface-card overflow-hidden">
      <header className="border-b border-border p-4 sm:p-5">
        ...
        <p className="text-meta mt-1">
          Move members and records from your old software. Nothing is saved until you confirm.
        </p>
        <ol
      ...
      <div className="grid gap-4 p-4 sm:p-5">
              ...
              {IMPORT_TYPES.map((t) => (
                <button
                  ...
                  className={cn(
                    ...
                    type === t ? "border-primary bg-primary/10" : "border-border hover:bg-accent",
                  )}
                >
                  {IMPORT_LABELS[t]}
                </button>
          ...
          <>
            <label className="flex min-h-40 cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-border p-6 text-center hover:bg-accent/40">
              ...
              <span className="text-meta">
                First row must contain column names. Any column names work.
              </span>
              <input
                type="file"
                accept=".csv,.xlsx,.xls"
        ...
        {step === 2 && parsed ? (
          <>
            <p className="text-sm">
              Read <b>{file?.name}</b>: <b>{parsed.rows.length}</b> rows,{" "}
              <b>{parsed.headers.length}</b> columns.
            </p>
            {parsed.sheetNames.length > 1 ? (
              <Select value={sheet} onValueChange={(v) => file && void read(file, v)}>
                <SelectTrigger className="w-full sm:w-64" aria-label="Sheet">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex flex-wrap gap-1.5">
              {parsed.headers.map((h) => (
                <span key={h} className="rounded-md bg-muted px-2 py-1 text-xs">
                  {h}
                </span>
              ))}
            </div>
        ...
        {step === 3 && parsed ? (
          <>
                ...
                <label key={f.key} className="grid gap-1 text-sm">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setMapping((m) => {
                        const n = { ...m };
                        if (v === NONE) delete n[f.key];
                        else n[f.key] = v;
                        return n;
                      })
                    }
                  >
                    <SelectTrigger className="w-full" aria-label={`Column for ${f.label}`}>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
            ...
            <div className="flex gap-2">
              ...
              <Button disabled={!!missingRequired.length} onClick={() => setStep(4)}>
                Preview
              </Button>
            </div>
          </>
        ...
        {step >= 4 && step <= 7 && parsed ? (
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {[
              ["Total rows", counts.total, "info"],
              ["Valid rows", counts.valid, "success"],
              ["Invalid rows", counts.invalid, "danger"],
              ["Possible duplicates", counts.dup, "warning"],
            ].map(([l, v]) => (
              <div key={l as string} className="rounded-xl border border-border p-3">
                <p className="text-meta">{l}</p>
        ...
        {step === 4 && parsed ? (
          <>
                ...
                <tbody>
                  {validated.slice(0, 200).map((r) => (
                    ...
                    >
                      ...
                      {previewCols.map((f) => (
                        <td key={f.key} className="max-w-40 truncate p-2">
                          {String(r.data[f.key] ?? "")}
                        </td>
                      ))}
                      <td className="p-2">
                        {r.errors.length ? (
                          <StatusPill tone="danger">{r.errors[0]!.reason}</StatusPill>
                        ) : r.duplicate ? (
                          <StatusPill tone="warning">Duplicate</StatusPill>
                        ) : (
            ...
            {validated.length > 200 ? (
              <p className="text-meta">Showing first 200 rows. All rows are validated.</p>
            ) : null}
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setStep(3)}>
                Back
              </Button>
              <Button onClick={() => setStep(5)}>Validate</Button>
            </div>
          </>
        ) : null}
        {step === 5 ? (
          <>
            {unknown.packages.length ? (
              <div className="grid gap-3 rounded-xl border border-warning/40 p-3">
                <p className="text-sm font-semibold">
                  Packages not found — choose what to do. Prices are never guessed.
                </p>
                {unknown.packages.map((name) => {
                  const r = packageRes[key(name)];
                  return (
                    <div
                      key={name}
                      className="grid gap-2 sm:grid-cols-[1fr_200px_1fr] sm:items-center"
                    >
                          ...
                          setPackageRes((m) => ({
                            ...
                            [key(name)]:
                              v === "create"
                                ...
                                : v === "skip"
                                  ? { mode: "skip" }
                                  : { mode: "map", id: packages.data[0]?.id ?? "" },
                          }))
                        }
                      >
                        <SelectTrigger className="w-full" aria-label={`Resolve package ${name}`}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="create">Create new package</SelectItem>
                      ...
                      {r?.mode === "map" ? (
                        <Select
                          value={r.id}
                          onValueChange={(id) =>
                            setPackageRes((m) => ({ ...m, [key(name)]: { mode: "map", id } }))
                          }
                        >
                          <SelectTrigger className="w-full" aria-label="Existing package">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            {packages.data.map((p) => (
            ...
            {unknown.trainers.length ? (
              <div className="grid gap-3 rounded-xl border border-border p-3">
                ...
                {unknown.trainers.map((name) => {
                  const r = trainerRes[key(name)];
                  return (
                    <div
                      key={name}
                      className="grid gap-2 sm:grid-cols-[1fr_200px_1fr] sm:items-center"
                    >
                          ...
                          setTrainerRes((m) => ({
                            ...
                            [key(name)]:
                              v === "create"
                                ? { mode: "create" }
                                : v === "map"
                                  ? { mode: "map", id: trainers.data[0]?.id ?? "" }
                                  : { mode: "skip" },
                          }))
                        }
                      >
                        <SelectTrigger className="w-full" aria-label={`Resolve trainer ${name}`}>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="skip">Skip trainer assignment</SelectItem>
                          <SelectItem value="create">Create trainer</SelectItem>
                          <SelectItem value="map" disabled={!trainers.data.length}>
                            Map to existing
                          </SelectItem>
                        </SelectContent>
                      </Select>
                      {r?.mode === "map" ? (
                        <Select
                          value={r.id}
                          onValueChange={(id) =>
                            setTrainerRes((m) => ({ ...m, [key(name)]: { mode: "map", id } }))
                          }
                        >
                          <SelectTrigger className="w-full" aria-label="Existing trainer">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            {trainers.data.map((t) => (
            ...
            {!unknown.packages.length && !unknown.trainers.length ? (
              <p className="flex items-center gap-2 text-sm">
                <CheckCircle2 className="size-4 text-success" /> All rows checked.{" "}
                {counts.invalid ? `${counts.invalid} row(s) have problems.` : "No problems found."}
              </p>
            ) : null}
            <div className="flex gap-2">
              ...
              <Button onClick={() => setStep(6)}>Review errors & duplicates</Button>
            </div>
          </>
        ) : null}
        {step === 6 ? (
          <>
            <div className="flex flex-wrap gap-2">
              {(["all", "invalid", "duplicates"] as const).map((f) => (
                <Button
                  key={f}
                  size="sm"
              ...
              {counts.invalid ? (
                <Button size="sm" variant="outline" onClick={() => failuresFor(null)}>
            ...
            <ul className="grid max-h-[460px] gap-2 overflow-auto">
              {shown
                .filter((r) => r.errors.length || r.duplicate || r.warnings.length)
                .map((r) => (
                  <li key={r.row} className="rounded-xl border border-border p-3 text-sm">
                    ...
                    {r.duplicate && !r.errors.length ? (
                      <div className="mt-2 grid gap-2 sm:grid-cols-[1fr_1fr_180px] sm:items-center">
                        <div>
                          <p className="text-meta">Existing</p>
                          <p>{r.duplicate.label}</p>
                        </div>
                        <div>
                          <p className="text-meta">Incoming</p>
                          <p>{r.duplicate.incoming || "—"}</p>
                        </div>
                        <Select
                          value={dupActions[r.row] ?? "skip"}
                          onValueChange={(v) =>
                            setDupActions((m) => ({ ...m, [r.row]: v as DupAction }))
                          }
                        >
                          <SelectTrigger
                            className="w-full"
                            aria-label={`Duplicate action row ${r.row}`}
                          >
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            ...
                            {r.duplicate.id &&
                            (type === "clients" || type === "packages" || type === "trainers") ? (
              ...
              {!validated.some((r) => r.errors.length || r.duplicate || r.warnings.length) ? (
                <li className="text-sm text-muted-foreground">No errors or duplicates.</li>
              ) : null}
            </ul>
            <div className="flex gap-2">
          ...
          <>
            <div className="rounded-xl border border-border bg-muted/40 p-4 text-sm">
              <p className="font-semibold">
                Ready to import {IMPORT_LABELS[type].toLowerCase()} from {file?.name}
              </p>
              <p className="mt-1">
                {toWrite} row(s) will be saved · {counts.invalid} invalid row(s) will not be
        ...
        {step === 8 ? (
          <p className="flex items-center gap-2 text-sm">
            <Loader2 className="size-4 animate-spin" /> Importing… {progress}
          </p>
        ) : null}
        {step === 9 && result ? (
          <>
            <div className="rounded-xl border border-border p-4">
              ...
              <dl className="mt-3 grid grid-cols-2 gap-2 text-sm sm:grid-cols-5">
                {[
                  ...
                  <div key={l as string}>
                    <dt className="text-meta">{l}</dt>
                    <dd className="text-lg font-bold">{v}</dd>
                  </div>
                ))}
              </dl>
              {result.updated ? (
                <p className="text-meta mt-2">{result.updated} existing record(s) updated.</p>
              ) : null}
            </div>
            <div className="flex flex-wrap gap-2">
              <Button asChild>
                <Link to={VIEW[type]}>View imported records</Link>
              </Button>
              <Button
                variant="outline"
                disabled={!result.failures.length}
                onClick={() => failuresFor(result)}
              >
