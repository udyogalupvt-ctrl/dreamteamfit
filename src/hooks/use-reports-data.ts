  ...
  const result = useMemo(() => {
    ...
    const sections: ExportableReportSection[] = [
      {
        ...
        metrics: [
          { id: "gross", label: "Gross Sales", value: String(grossSales) },
          { id: "collected", label: "Amount Collected", value: String(collected) },
          { id: "outstanding", label: "Outstanding", value: String(outstanding) },
          { id: "refunded", label: "Refunded", value: String(refunded) },
        ],
        rows: rangedInvoices.map((i) => ({
          invoiceNumber: i.invoiceNumber,
          date: i.invoiceDate,
          client: i.clientNameSnapshot,
          total: i.total,
      ...
      },
      {
        ...
        metrics: [{ id: "total", label: "Total Expenses", value: String(expenseTotal) }],
      ...
      },
      {
        ...
        metrics: [],
        rows: rangedInquiries.map((i) => ({
          name: i.name,
          status: i.status,
          source: i.source,
          createdAt: dateOf(i.createdAt),
        })),
      },
      {
        id: "bookings",
        ...
        metrics: [],
        rows: rangedBookings.map((b) => ({
          date: b.date,
          type: b.bookingType,
          client: b.clientNameSnapshot,
          status: b.status,
        })),
      },
      {
        id: "workout-diet",
        title: "Workout & Diet Report",
        metrics: [],
        rows: [
          ...rangedWorkouts.map((w) => ({
            type: "Workout",
            plan: w.planNameSnapshot,
            status: w.status,
            assignedDate: w.assignedDate,
          })),
          ...rangedDiets.map((d) => ({
            type: "Diet",
            plan: d.planNameSnapshot,
            status: d.status,
      ...
      },
      {
        ...
        rows: rangedAttendance.map((a) => ({
          ...
          member: a.clientNameSnapshot,
          biometricId: a.biometricUserId,
          event: a.eventType,
          device: a.deviceNameSnapshot,
          decision: a.accessDecision,
          reason: a.accessReason,
          source: a.source,
        })),
      },
    ];
    return {
      ...
      clients: {
        ...
        active,
        expired,
        newClients: clients.data.filter((c) => inRange(dateOf(c.createdAt))).length,
        newMemberships: memberships.data.filter((m) => inRange(dateOf(m.createdAt))).length,
        upcoming,
      },
