import { doc,orderBy,runTransaction,serverTimestamp,updateDoc,type DocumentData } from "firebase/firestore";
...
export async function sendWhatsAppMessage(input:SendInput){
  if(input.type!=="test"&&!input.client.whatsappOptIn)throw new Error("This client has not opted in to WhatsApp messages.");
  const normalized=normalizeWhatsAppPhone(input.client.whatsappPhone||input.client.phone);if(!normalized.ok)throw new Error(normalized.error);
  const id=safeId(`${input.type}__${input.referenceId}`),ref=doc(db,COLLECTIONS.whatsappMessages,id);
  const claimed=await runTransaction(db,async tx=>{const existing=await tx.get(ref);if(existing.exists()&&["queued","sent","delivered","read"].includes(String(existing.data()["status"])))return false;const now=serverTimestamp();tx.set(ref,{clientId:input.client.id,clientNameSnapshot:input.client.fullName,phoneSnapshot:input.client.whatsappPhone||input.client.phone,normalizedPhone:normalized.value,type:input.type,referenceId:input.referenceId,provider:input.provider,templateName:input.templateName,templateLanguage:input.templateLanguage??"en",messagePreview:input.messagePreview,status:"queued",providerMessageId:"",sentAt:null,deliveredAt:null,readAt:null,failedAt:null,errorCode:"",errorMessage:"",createdAt:existing.data()?.["createdAt"]??now,updatedAt:now},{merge:true});return true});
