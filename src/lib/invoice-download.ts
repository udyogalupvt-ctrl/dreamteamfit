...
/** Public PDF link served by the "invoicePdf" cloud function (used as the WhatsApp attachment). */
export function invoicePdfUrl(publicToken: string) {
  return `https://us-central1-${firebaseConfig.projectId ?? ""}.cloudfunctions.net/invoicePdf?t=${encodeURIComponent(publicToken)}`;
}
